...
# Check if correct
execute as @e[tag=lock_red] at @s if score @s randomize_lock = @p lock_pick_number run playsound minecraft:block.copper_bulb.turn_on block @a ~ ~ ~ 10 0.5
...
execute as @e[tag=lock_red,scores={lock_difficulty=0}] at @s run playsound block.chain.break block @a ~ ~ ~ 10 1
